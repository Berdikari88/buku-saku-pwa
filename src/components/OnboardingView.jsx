...
export default function OnboardingView({ session, onComplete }) {
  ...
  // State full_name murni diisi oleh user di halaman ini
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // FIXED: Menggunakan .upsert() untuk mencegah error duplikasi Primary Key
    ...
    <div className="min-h-screen bg-[#E6E2DE] text-[#1E1E1E] font-sans flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#FDFBF7] border-8 border-[#1E1E1E] rounded-[32px] p-6 sm:p-8 shadow-[12px_12px_0px_0px_#1E1E1E] animate-in slide-in-from-bottom-4">
        ...
        <div className="mb-8 text-center space-y-2">
          ...
          <p className="text-[10px] font-bold text-stone-500 uppercase tracking-widest leading-relaxed">Bantu Buku Saku mengenalmu lebih dekat untuk pengalaman yang lebih personal.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input type="text" name="full_name" required value={formData.full_name} onChange={handleChange} placeholder="NAMA LENGKAP" className="w-full pl-11 pr-4 py-3.5 text-xs border-4 border-[#1E1E1E] rounded-2xl font-bold bg-white focus:border-indigo-500 outline-none transition uppercase" />
            </div>
          </div>
